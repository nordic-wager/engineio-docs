<script lang="ts">
	...
	const combinedSearchIndex: SearchEntry[] = [
		...searchIndex,
		...faqSearchIndex,
		...changelogSearchIndex
	];
...
<header class="sticky top-0 z-30 bg-zinc-950/90 backdrop-blur-xl">
	...
	<div class="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 lg:px-6">
		...
		<div class="flex items-center gap-6">
			<button
				onclick={onMenuClick}
				class="rounded-lg p-2 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-white lg:hidden"
			...
			<nav class="ml-4 hidden items-center gap-1 lg:flex">
				<a
					href="{base}/docs"
					class="nav-link {!isFaqSection && !isChangelogSection ? 'active' : ''}">Docs</a
				>
				<a href="{base}/faq" class="nav-link {isFaqSection ? 'active' : ''}">FAQ</a>
				<a href="{base}/changelog" class="nav-link {isChangelogSection ? 'active' : ''}"
					>Changelog</a
				>
			</nav>
		</div>
		...
		<div class="flex items-center gap-3">
			<button
				onclick={openSearch}
				class="hidden w-56 items-center gap-2 rounded-lg border border-zinc-800 bg-zinc-900 px-4 py-2 text-sm text-zinc-400 transition hover:border-zinc-700 hover:text-zinc-300 sm:flex"
			>
			...
			<button
				onclick={openSearch}
				class="rounded-lg p-2 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-white sm:hidden"
				aria-label="Search"
			>
				<svg class="h-5 w-5" fill="none" viewBox="0 0 24 24" stroke="currentColor">
					<path
			...
			<a
				...
				class="rounded-lg p-2 text-zinc-400 transition-colors hover:bg-zinc-800 hover:text-[#5865F2]"
				aria-label="Discord"
			>
	...
	>
		...
		>
			...
			<div class="max-h-80 overflow-y-auto">
				{#if query.trim() === ''}
					<div class="p-4 text-center text-sm text-zinc-500">Start typing to search...</div>
				...
				{:else}
					<ul class="py-2">
						{#each results as result, i}
							<li>
								<button
									class="flex w-full flex-col gap-0.5 px-4 py-2.5 text-left transition {i ===
									selectedIndex
										? 'bg-white/[0.06]'
										: 'hover:bg-white/[0.03]'}"
								...
								>
									...
									>
										{#each highlightParts(result.title, query.trim().toLowerCase()) as part}
											{#if part.match}<mark class="rounded-sm bg-amber-500/30 px-0.5 text-amber-200"
													>{part.text}</mark
												>{:else}{part.text}{/if}
										{/each}
									</span>
									...
									{#if result.snippet}
										<span class="line-clamp-1 text-xs text-zinc-500">
											{#each highlightParts(result.snippet, query.trim().toLowerCase()) as part}
												{#if part.match}<mark
														class="rounded-sm bg-amber-500/30 px-0.5 text-amber-200"
														>{part.text}</mark
													>{:else}{part.text}{/if}
											{/each}
										</span>
